...
if __name__ == "__main__":
    ...
    parser.add_argument('-hierarchy', '--hierarchy', type=bool)
    parser.add_argument('-strength', '--strength', type=bool)
    parser.add_argument('-recency', '--recency', type=bool)
    ...
    ### drop unused columns
    train_x = train_x.drop(columns=["ID", "Year","Team","final_position"])
    valid_x = valid_x.drop(columns=["ID", "Year","Team","final_position"])
    test_x = test_x.drop(columns=["ID", "Year","Team","final_position"])
    ...
    for seed in range(0,10):
        ...
        model = Nonhier_Optmatch(indiv_f_dim = len(individual_features),
        ...
        for epoch in range(epochs):
            ...
            train_y_hat = model(indiv_f=normalized_train_indiv,
            ...
            with torch.no_grad():
                ...
                valid_y_hat = model(indiv_f=normalized_valid_indiv,
                ...
                test_y_hat = model(indiv_f=normalized_test_indiv,
